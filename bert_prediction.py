...
def get_bert_prediction(train_pos_fasta, train_neg_fasta, test_pos_fasta, test_neg_fasta, model_name, model_path, MAX_LEN):
    ...
    for epoch_i in range(0, epochs):
        ...
        file_name = 'ensemble_prot_bert_bfd_epcoh'+str(epoch_i+1)+'_'+str(learning_rate)+'.pt'
    ...
    model = torch.load(model_path + '/ensemble_prot_bert_bfd_epcoh2_1e-06.pt')
...
#    print('Max sentence length: ', max([len(sen) for sen in input_ids]))
    ...
    for epoch_i in range(0, epochs):
        ...
        print("Saving model to %s" % output_dir)
        file_name = str(model_name)+'prot_bert_bfd_epcoh'+str(epoch_i+1)+'_'+str(learning_rate)+'.pt'
        torch.save(model,os.path.join(output_dir, file_name))
...
#        test_attention_masks.append(seq_mask) 
    ...
    # Evaluate on Test Set
    # With the test set prepared, we can apply our fine-tuned model to generate predictions on the test set.
    model = torch.load(model_path + '/' + model_name + 'prot_bert_bfd_epcoh2_1e-06.pt')
