...
def train_doc2vec_model(train_data, train_label, validation_data, validation_label, model_name, path = None):
    ...
    best_weights_filepath = os.path.join(path,'%s_best_weights_DS2.h5'%model_name)
    saveBestModel = tensorflow.keras.callbacks.ModelCheckpoint(best_weights_filepath, 
                                                        monitor='val_loss', 
                                                        verbose=1, 
                                                        save_best_only=True, 
                                                        mode='auto')
    CSVLogger = tensorflow.keras.callbacks.CSVLogger(os.path.join(path, "%s_csvLogger_DS2.csv"%model_name) ,separator=',', append=False)
    ...
    history.model.save(os.path.join(path,'%s_final_weights_DS2.h5'%model_name))
